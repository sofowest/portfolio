...
function useFlickerTrigger() {
  ...
  const [finalTransparent, setFinalTransparent] = useState(false);
  // Recibe el trigger desde Home y la duración
  const triggerFlicker = (duration = 900) => {
    setFlickerBg(true);
    setFinalTransparent(false);
    setTimeout(() => {
      setFlickerBg(false);
      setFinalTransparent(true);
    }, duration);
  };
  return [flickerBg, finalTransparent, triggerFlicker];
}


function App() {
  const [flickerBg, finalTransparent, triggerFlicker] = useFlickerTrigger();
